					   ...
					   <tr align='center'><td>";
						 ...
						 while($row=mysql_fetch_row($result,MYSQL_BOTH) and $j<=$records_per_page)
						 {
								 echo"<table border='1' align='left' width='33.5%' height='80px'>
									  ...
									  $k++;																			
								//以"加入購物車"來決定傳送的值
								echo"<form name='form1' method='post' action='membshop.php'>";
								   echo"<tr><td><input type='hidden' name='car[]' value='".$row["goid"]."'>
								                <input type='submit' name='".$row["goid"]."' value='加入購物車'></td></tr>";
								echo"</form>";
                                $k=$row["goid"];								
								if(@$k=$_POST[$k])
								{
